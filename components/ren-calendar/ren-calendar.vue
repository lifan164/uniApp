<template>
	<view class="calendar-wrapper">
		<view class="header" v-if="headerBar">
			<view class="pre" @click="changeMonth('pre')"> 前 </view>
			<view>{{y+'年'+formatNum(m)+'月'}}</view>
			<view class="next" @click="changeMonth('next')"> 后 </view>
		</view>
		<view class="week">
		...
		<view :class="{ hide: !monthOpen }" class="content" :style="{ height: height }">
			<view :style="{ top: positionTop + 'rpx' }" class="days">
				...
				v-bind:class="[isMarkDay(item.year, item.month, item.date).class]">
					...
					<!-- <view class="markDay" v-if="isMarkDay(item.year, item.month, item.date).flag&&item.isCurM"></view> -->
		...
		methods: {
			...
			isMarkDay(y, m, d) {
				...
				for (let i = 0; i < this.markDays.length; i++) {
					let dy = `${y}-${m}-${d}`;
					if (this.markDays[i].time == dy) {
						returnObj.flag = true;
						returnObj.class = this.markDays[i].showClass
						break;
					}
				}
				return returnObj;
			...
			changeMonth(type) {
				...
				this.$emit('changeMonth', changeType)
			}
		}
	};
</script>
...
<style lang="scss">
	// @import url("../../pages/energyEfficiency/machineRoom/machineRoom.css");
	.redClass {
		color: #FF0005!important;
	}
	
	.yellowClass {
		color: #F5D812!important;
	}
